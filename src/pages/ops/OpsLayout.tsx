...
export default function OpsLayout() {
  ...
  // Enable push on first visit if admin
  useEffect(() => {
    if (user && isAdmin && !pushEnabled) {
      subscribeToPush(user.id).then(setPushEnabled);
    }
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'settings': return <OpsSettings pushEnabled={pushEnabled} onEnablePush={() => subscribeToPush(user!.id).then(setPushEnabled)} />;
    }
  };
      ...
      {showInstallBanner && (
        <div className="bg-washero-charcoal text-primary px-4 py-3 flex items-center justify-between gap-3 safe-top">
          ...
          <div className="flex gap-2">
            <Button size="sm" variant="ghost" onClick={() => setShowInstallBanner(false)} className="text-muted-foreground h-8 px-2">
              Luego
            </Button>
        ...
        <div className="flex items-center gap-2">
          {!pushEnabled && (
            <Button 
              ...
              onClick={() => subscribeToPush(user!.id).then(setPushEnabled)}
            >
